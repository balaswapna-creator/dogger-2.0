    ...
    <div v-else-if="patient" class="patient-detail">
        ...
        <div class="info-grid">
          ...
          <div class="info-details">
            ...
            <div class="detail-row">
              ...
              <span class="value">{{ calculateDetailedAge(patient.date_of_birth) }}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
              ...
              <span class="value">{{ formatDate(patient.created_at || patient.date_of_registration) }}</span>
            </div>
            <div class="detail-row" v-if="patient.microchip_number || patient.microchip_id">
              <span class="label">Microchip:</span>
              <span class="value">{{ patient.microchip_number || patient.microchip_id }}</span>
            </div>
          </div>
        </div>

        <!-- Owner Information -->
        <div class="owner-section" v-if="patient.owner_name || patient.owner">
          <h3>👤 Owner Information</h3>
          <div class="detail-row">
            <span class="label">Name:</span>
            <span class="value">{{ patient.owner_name || (patient.owner ? patient.owner.name : 'N/A') }}</span>
          </div>
          <div class="detail-row" v-if="patient.owner_phone || (patient.owner && patient.owner.phone)">
            <span class="label">Phone:</span>
            <span class="value">{{ patient.owner_phone || patient.owner.phone }}</span>
          </div>
          <div class="detail-row" v-if="patient.owner_email || (patient.owner && patient.owner.email)">
            <span class="label">Email:</span>
            <span class="value">{{ patient.owner_email || patient.owner.email }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="info-card">
        ...
        <div class="stats-grid">
          <div class="stat-item">
            <div class="stat-label">Latest Vaccination</div>
            <div class="stat-value">{{ latestVaccination }}</div>
          </div>
          <div class="stat-item">
        ...
        <div v-else class="records-list">
          <div v-for="record in medicalRecords" :key="record.id" class="record-item">
            ...
            <div class="record-details">
              ...
              <div class="record-complaint">{{ record.chief_complaint || record.diagnosis || 'N/A' }}</div>
              <div class="record-diagnosis">{{ record.treatment || '' }}</div>
            </div>
      ...
      <div class="vaccinations-section info-card">
        ...
        <div v-else class="vaccination-list">
          <div v-for="vacc in vaccinations" :key="vacc.id" class="vaccination-item">
            ...
            <div class="vacc-date">{{ formatDate(vacc.vaccination_date || vacc.date_administered) }}</div>
      ...
      <div class="payments-section info-card">
        ...
        <div v-else class="payment-list">
          <div v-for="payment in payments" :key="payment.id" class="payment-item">
            ...
            <div class="payment-amount">₹{{ parseFloat(payment.total_amount || payment.amount || 0).toFixed(2) }}</div>
            <div class="payment-method">{{ payment.payment_method || 'Cash' }}</div>
            <div class="payment-status" :class="payment.payment_status || payment.status">
...
export default {
  ...
  setup() {
    ...
    // ✅ FIXED: Calculate detailed age (years + months)
    ...
    // ✅ FIXED: Latest vaccination display
    const latestVaccination = computed(() => {
      if (!vaccinations.value || vaccinations.value.length === 0) {
        return 'No vaccinations recorded'
      }
      
      const sorted = [...vaccinations.value].sort((a, b) => 
        new Date(b.vaccination_date || b.date_administered) - new Date(a.vaccination_date || a.date_administered)
      )
      
      const latest = sorted[0]
      const date = formatDate(latest.vaccination_date || latest.date_administered)
      return `${latest.vaccine_name} - ${date}`
    })

    // ✅ FIXED: Total payments calculation
    const totalPayments = computed(() => {
      ...
      const total = payments.value.reduce((sum, payment) => 
        sum + parseFloat(payment.total_amount || payment.amount || 0), 0
      )
    ...
    // ✅ FIXED: Updated createPassbook to use patient_id
    const createPassbook = async () => {
      ...
      try {
        ...
        const response = await api.post('/passbooks/', {
          patient_id: patient.value.id,  // Changed from 'patient' to 'patient_id'
          is_active: true
        })
        ...
        const errorMsg = err.response?.data?.error || err.response?.data?.detail || err.message || 'Failed to create passbook'
